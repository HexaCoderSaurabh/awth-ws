...
@Injectable()
export class GetUserPipe implements PipeTransform<string, Promise<Users | null>> {

  private readonly logger = new Logger('GetUserPipe')

  constructor(private readonly userService: UserService) { }

  async transform(value: string, metadata: ArgumentMetadata): Promise<Users | null> {
    try {
      const user = await this.userService.findUserByName(value)
      if( !user) {
